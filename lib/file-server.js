...
module.exports.createServer = function (
    ...
    app.use(function(req, res, next){
        res.setHeader('Feature-Policy', 'fullscreen *');
        res.setHeader('Feature-Policy', 'accelerometer *');
        res.setHeader('Feature-Policy', 'autoplay *');
        res.setHeader('Feature-Policy', 'document-domain *');
        res.setHeader('Sec-Fetch-Site', 'cors');
        next();
    })
    ...
    app.use('/', serveIndex(uploadPath, {icons:true}));
    ...
    app.use(function (err, req, res, next) {
        if (err) {
            logger.info(
                ...
                colors.red(String(err.status)), colors.red(err.message)
            );
        }
        res.status(err.status || 404).send( err.message || "Sorry can't find that!")
